            ...
            for (i1 = 0; i1 < 3; i1++)
                if (min_y + i1 < 16)
                    for (i2 = 0; i2 < 3; i2++)
                    {
                        s <<= 1;
                        if (min_x + i2 < 16)
                            if (key_map[(min_y + i1) * 16 + (min_x + i2)])
                                s |= 1;
                    }
            // print_dbg("\r\nfound shape pattern: ");
            // print_dbg_ulong(s);
...
void rec_stop()
{
    ...
    // set final length
    if (rec_timer)
        es.p[p_select].e[rec_position - 1].interval = rec_timer - 1;
    else
        es.p[p_select].e[rec_position - 1].interval = 1;
...
void pattern_time_half()
{
    ...
    for (i = 0; i < es.p[p_select].length; i++)
    {
        ...
        if (!es.p[p_select].e[i].interval)
            es.p[p_select].e[i].interval = 1;
    }
...
static void refresh()
{
    ...
    if (r_status == rArm)
        ...
        monomeLedBuffer[32] = 11 + 4 * (blinker < 24);

    // LOOP and MODE
    if (es.p[p_select].loop)
        monomeLedBuffer[48] = 11;
    if (arp)
        monomeLedBuffer[64] = 11;

    // PATTERN PLAY MODE
    if (mode == mBank)
        monomeLedBuffer[16] = 11;
    ...
    else if (es.p[p_select].length != 0)
        monomeLedBuffer[0] = 4;

    // PORT INDICATOR
    if (port_toggle)
        monomeLedBuffer[112] = port_toggle * 4 + 3;
    ...
    else if (mode == mSelect || mode == mBank)
    {
        for (i1 = 0; i1 < 4; i1++)
            for (i2 = 0; i2 < 4; i2++)
                if (es.p[i1 * 4 + i2].length)
                    monomeLedBuffer[i1 * 16 + i2 + 34] = 7;
                else
                    monomeLedBuffer[i1 * 16 + i2 + 34] = 4;
    ...
    else
    {
        ...
        else if (!singled)
        {
            ...
            {
                if (root_y > 0)
                    monomeLedBuffer[(root_y - 1) * 16 + root_x] = 11;
            }
            else if (shape_on == 1)
            {
                if (root_y > 0)
                    monomeLedBuffer[(root_y - 1) * 16 + root_x + 1] = 11;
            }
            else if (shape_on == 2)
                monomeLedBuffer[(root_y)*16 + root_x + 1] = 11;
            else if (shape_on == 3)
                monomeLedBuffer[(root_y + 1) * 16 + root_x + 1] = 11;
            else if (shape_on == 4)
            {
                if (root_y > 0)
                    monomeLedBuffer[(root_y - 1) * 16 + root_x] = 11;
                if (root_y > 1)
                    monomeLedBuffer[(root_y - 2) * 16 + root_x] = 11;
            }
            else if (shape_on == 5)
            {
                if (root_y > 0)
                    monomeLedBuffer[(root_y - 1) * 16 + root_x + 1] = 11;
                if (root_y > 1)
                    monomeLedBuffer[(root_y - 2) * 16 + root_x + 2] = 11;
            }
            else if (shape_on == 6)
            {
                monomeLedBuffer[(root_y)*16 + root_x + 1] = 11;
                monomeLedBuffer[(root_y)*16 + root_x + 2] = 11;
            }
            else if (shape_on == 7)
            {
                monomeLedBuffer[(root_y + 1) * 16 + root_x + 1] = 11;
    ...
    // MODIFIERS/MODE
    if (mode == mEdge)
        monomeLedBuffer[80] = 15;
    if (mode == mSlew)
        monomeLedBuffer[96] = 15;
...
static void refresh_mono()
{
    ...
    if (r_status == rArm)
        ...
        monomeLedBuffer[32] = 15 * (blinker < 24);

    // LOOP and MODE
    if (es.p[p_select].loop)
        monomeLedBuffer[48] = 15;
    if (arp)
        monomeLedBuffer[64] = 15;
    ...
    else
    {
        ...
        {
            if (shape_on == 0)
                monomeLedBuffer[(root_y - 1) * 16 + root_x] = 15;
            else if (shape_on == 1)
            {
                if (root_y > 0)
                    monomeLedBuffer[(root_y - 1) * 16 + root_x + 1] = 15;
            }
            else if (shape_on == 2)
                monomeLedBuffer[(root_y)*16 + root_x + 1] = 15;
            else if (shape_on == 3)
                monomeLedBuffer[(root_y + 1) * 16 + root_x + 1] = 15;
            else if (shape_on == 4)
            {
                if (root_y > 0)
                    monomeLedBuffer[(root_y - 1) * 16 + root_x] = 15;
                if (root_y > 1)
                    monomeLedBuffer[(root_y - 2) * 16 + root_x] = 15;
            }
            else if (shape_on == 5)
            {
                if (root_y > 0)
                    monomeLedBuffer[(root_y - 1) * 16 + root_x + 1] = 15;
                if (root_y > 1)
                    monomeLedBuffer[(root_y - 2) * 16 + root_x + 2] = 15;
            }
            else if (shape_on == 6)
            {
                monomeLedBuffer[(root_y)*16 + root_x + 1] = 15;
                monomeLedBuffer[(root_y)*16 + root_x + 2] = 15;
            }
            else if (shape_on == 7)
            {
                monomeLedBuffer[(root_y + 1) * 16 + root_x + 1] = 15;
